    ...
    void PerformRequest(TEvKqp::TEvCompileRequest::TPtr& ev, const TActorContext& ctx) {
        ...
        TKqpCompileSettings compileSettings(
            ...
            ev->Get()->Split
                ...
                : ((TableServiceConfig.GetEnableAstCache() || request.Query->Settings.RequireIdempotency) && !request.QueryAst)
        ...
        if ((TableServiceConfig.GetEnableAstCache() || compileRequest.Query.Settings.RequireIdempotency) && request.QueryAst) {
    ...
    void PerformRequest(TEvKqp::TEvRecompileRequest::TPtr& ev, const TActorContext& ctx) {
        ...
        if (compileResult || request.Query) {
            ...
            TKqpCompileSettings compileSettings(
                ...
                ev->Get()->Split
                    ? ECompileActorAction::SPLIT
                : ((TableServiceConfig.GetEnableAstCache() || query.Settings.RequireIdempotency) && !request.QueryAst)
                        ? ECompileActorAction::PARSE
                        : ECompileActorAction::COMPILE);
            if (compileResult) {
