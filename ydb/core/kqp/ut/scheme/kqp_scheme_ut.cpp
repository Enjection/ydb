...
Y_UNIT_TEST_SUITE(KqpOlapTypes) {
    ...
    Y_UNIT_TEST(BackupRequestIdempotencyReplaysAndChecksDualKeys) {
        ...
        const auto mismatch = ExecuteUidQuery(kikimr, query, TString("backup:other"));
        ...
        for (ui32 attempt = 0; attempt != 2; ++attempt) {
            TMaybe<TString> uid;
            if (attempt == 0) {
                uid = "ключ с пробелами/и?символами!";
            }
            const auto result = ExecuteUidQuery(kikimr, query, uid);
            UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
            TResultSetParser parser(result.GetResultSet(0));
            UNIT_ASSERT(parser.TryNextRow());
            const auto id = parser.ColumnParser("operation_id").GetUtf8();
