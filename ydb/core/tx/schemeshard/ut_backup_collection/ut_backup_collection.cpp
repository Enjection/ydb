...
Y_UNIT_TEST_SUITE(TBackupCollectionTests) {
    ...
    Y_UNIT_TEST(IdempotencyRedoRejectionReleasesUidForAllKinds) {
        TTestBasicRuntime runtime;
        TTestEnv env(runtime, TTestEnvOptions().EnableBackupService(true));
        ui64 txId = 100;
        PrepareDirs(runtime, env, txId);
        ...
        for (const auto& [kind, ddl] : operations) {
            ...
            if (kind == NKikimrSchemeOp::ESchemeOpBackupIncrementalBackupCollection) {
                ...
                UNIT_ASSERT_VALUES_EQUAL(TestGetIncrementalBackup(runtime, originalId, "/MyRoot").GetIncrementalBackup().GetStatus(),
                    Ydb::StatusIds::SUCCESS);
            ...
            if (kind == NKikimrSchemeOp::ESchemeOpBackupBackupCollection) {
                ...
                runtime.SendToPipe(TTestTxConfig::SchemeShard, sender,
                    new TEvBackup::TEvGetFullBackupRequest("/MyRoot", rejectedId), 0, GetPipeConfigWithRetries());
                ...
                TestGetIncrementalBackup(runtime, rejectedId, "/MyRoot", Ydb::StatusIds::NOT_FOUND);
            } else {
